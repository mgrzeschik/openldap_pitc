...
# The push triggers the Copr webhook, which rebuilds for EL9 and EL10.
set -Eeuo pipefail
...
echo "latest:  $new"
if [ "$new" = "$cur" ]; then echo "Up to date, nothing to do."; exit 0; fi

echo "Sanity check: preparing sources for $new"
scripts/prepare-sources.sh "$new" _work
...
if [ "$PUSH" = 1 ]; then
  ...
  git push
  echo "Bumped ${cur:-<none>} -> $new and pushed."
else
  echo "Updated UPSTREAM_TAG to $new (not committed)."
fi
